...
export default async function createZip(destination, options) {
	...
	try {
		...
		if (!fs.existsSync(configPath)) {
			spinner.fail();
			console.log(chalk.red(`zippie: ❌ Configuration file not found: ${configPath}`));
			return;
		}
		...
		if (patterns.length === 0) {
			spinner.fail();
			console.log(chalk.red('zippie: ❌ No patterns found in configuration file.'));
			return;
		}

		if (verbose) {
			spinner.info(`zippie: Found ${patterns.length} patterns in configuration file.`);
			spinner.start();
		}
		...
		for (const pattern of patterns) {
			if (pattern.includes(':!')) {
				...
				if (verbose) {
					spinner.info(`zippie: Pattern "${pattern}" matched ${filteredFiles.length} files.`);
					spinner.start();
				}
			} else {
				...
				if (verbose) {
					spinner.info(`zippie: Pattern "${pattern}" matched ${files.length} files.`);
					spinner.start();
				}
			}
		}
		...
		spinner.succeed();
		console.log(chalk.green(`zippie: ✅ Archive created successfully: ${destination}`));
		console.log(chalk.green(`zippie: Total files: ${chalk.cyan(totalFiles)}`));
		console.log(chalk.green(`zippie: Archive size: ${chalk.cyan(formatBytes(size))}`));

	} catch (error) {
		spinner.fail();
		console.error(chalk.red(`zippie: ❌ Error creating zip: ${error.message}`));
		if (verbose) {
			console.error(error);
		}
		process.exit(1);
